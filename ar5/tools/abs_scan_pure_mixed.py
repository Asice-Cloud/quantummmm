#!/usr/bin/env python3
"""Pure mixed (c_xy,c_yx) scan for ABS-like near-zero modes.

Runs a grid over c_xy and c_yx with c_xx=c_yy=0 and no Z terms.
Saves CSV and candidates JSON to results/.
"""
import os
import json
import numpy as np
...
def main():
    ...
    # base mapping can be zero since we set c_xx=c_yy=0 and no base XX/YY
    for cxy in cxy_vals:
        for cyx in cyx_vals:
            c_xx = 0.0
            c_yy = 0.0
    ...
    csv_path = os.path.join('results', 'abs_scan_pure_mixed.csv')
    with open(csv_path, 'w') as f:
        f.write('cxy,cyx,min_abs,site,max_weight,maj_sim\n')
        for r in out_rows:
            f.write(','.join(str(x) for x in r) + '\n')

    json_path = os.path.join('results', 'abs_scan_pure_mixed_candidates.json')
    with open(json_path, 'w') as f:
        json.dump(candidates, f, indent=2)

    print('Pure-mixed scan complete. CSV:', csv_path)
