...
| 第三层 | Riccati 变量 `q = Z X^{-1}` | **坐标选择** | 局部有效，在 `X` 可逆处严格等价 |
...
$$
\dot q = \dot Z X^{-1} - Z X^{-1} \dot X X^{-1}.
$$
...
$$
\begin{aligned}
\dot q &= (C X + D Z)X^{-1} - Z X^{-1}(A X + B Z)X^{-1} \\
       &= C + D(ZX^{-1}) - (ZX^{-1})A - (ZX^{-1})B(ZX^{-1}) \\
       &= C + D q - q A - q B q.
\end{aligned}
$$

得到一般块矩阵 Riccati 方程（不预设 sp(2) 约束）：

$$
\boxed{\dot q = C + D q - q A - q B q}.
$$

当 $K(t)$ 满足 sp(2) 条件 $C=-\bar B,\; D=-A$ 时，退化为
$\dot q = -\bar B - A q - q A - q B q$。
